...
**An operator looks at a wide array of data and makes decisions about chemical dosing.**
   
**OUR GOAL: Create a model that serves as an "operator," taking raw water data and making predictions of chemical doses.**
...
- **Raw Water Data**
   ...
   - Turbidity
   - Conductivity
   - Suspended Grain Size (dropped)
   - Alkalinity (dropped)
   - Hardness (dropped)
- **Chemical Dosing Data**
   - Ferric Chloride Dose: primary
   - Cationic Polymer Dose: secondary
- Data Cleaning
   - Removed significant outliers
   - Took daily averages for raw water data
   - Merged common dates between datasets
...
Introduced **lagged variables** like in the Stock dataset from the homework.  
   
- For each of the main four variables:
   - Introduce two lagged variables
   - First represents the day before
   - Second represents two days before
   
Introduced a **categorical predictor** indicating whether the dose increased, decreased, or stayed the same compared to the day before.
