...
week_data <- week_data %>% mutate(CollegePrice = case_when(
  row_number() <= 71 ~ 0.30,
  row_number() > 71 & row_number() <= 154 ~ 0.40,
  row_number() > 154 ~ 0.30
  )
) %>% mutate(NormalPrice = case_when(
  row_number() <= 71 ~ 0.30,
  row_number() > 71 ~ 0.40
  )
)

# have to remake test/train
week_data$Class <- if_else(week_data$year == 2024, 'Test', 'Train')
train <- subset(week_data, Class == 'Train') %>% arrange(year, WeekNum)
test <- subset(week_data, Class == 'Test') %>% arrange(year, WeekNum)
nrow(train)
...
Boosting
```{r}
boost_spec <- boost_tree(trees = 5000, tree_depth = 5, learn_rate = 0.1) %>% #0.1 is better than 0.01
  set_engine("xgboost") %>% 
  set_mode("regression")
