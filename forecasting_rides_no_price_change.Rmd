...
foco <- st_read("~/GitHub/D445_Project/Street_Centerlines")
...
a <- leaflet(join_data) %>%
  ...
  addPolylines(color = ~colorNumeric(rev("viridis"), Total_Matched_Trips)(Total_Matched_Trips),
               weight = 2,
               opacity = 0.9,
  ...
  addLegend(position = "bottomright", 
            pal = colorNumeric(rev("viridis"), 
                               join_data$Total_Matched_Trips), 
            values = join_data$Total_Matched_Trips, 
            title = HTML("First Price Period<br>Total Matched Trips")
            )
# a
```
```{r}
join_data2 <- st_transform(join_data3, crs = 4326)
...
b <- leaflet(join_data2) %>%
  ...
  addLegend(position = "bottomright", 
            pal = colorNumeric(rev("viridis"), 
                               join_data2$Total_Matched_Trips), 
            values = join_data2$Total_Matched_Trips, 
            title = HTML("Second Price Period<br>Total Matched Trips")
            )
# b
```
```{r}
...
c <- leaflet(join_data3) %>%
  ...
  addLegend(position = "bottomright", 
            pal = colorNumeric(rev("viridis"), 
                               join_data3$Total_Matched_Trips), 
            values = join_data3$Total_Matched_Trips, 
            title = HTML("Third Price Period<br>Total Matched Trips")
            )
c
```
...
arima_forecast <- forecast(arima_model, h = nrow(future_dates))
